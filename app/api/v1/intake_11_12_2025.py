...
from app.schemas.intake import IntakeCaseCreate, IntakeCaseResponse, IntakeQueueResponse
...
async def get_intake_queue(
    ...
    current_user: dict = Depends(AuthService.get_current_user_from_jwt)  # FIXED: Use JWT dependency
):
    """
    ...
    try:
        ...
        queue_response = await IntakeService.get_queue_items(
            page=page,
            page_size=page_size,
            status=status,
            source=source,
